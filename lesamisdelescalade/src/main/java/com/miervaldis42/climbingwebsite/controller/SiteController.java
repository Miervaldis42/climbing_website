...
public class SiteController {
	...
	public String editSiteDetails
		(@RequestParam("siteId") String siteId,
		 @RequestParam("siteTag") Optional<String> siteTag,
		 @RequestParam("siteDesc") Optional<String> siteDesc,
		 ...
		 @RequestParam("lengthRoute") Optional<int[]> lengthRouteIds)
	{
		...
		// Save site info
		if(siteTag.isPresent() || siteDesc.isPresent()) {
			Boolean tag = Boolean.parseBoolean(siteTag.get());
			associatedSite.setTag(tag);
			
			if(!siteDesc.get().isEmpty()) {
				associatedSite.setDescription(siteDesc.get());
			}
			
			siteService.saveSite(associatedSite);
		}
